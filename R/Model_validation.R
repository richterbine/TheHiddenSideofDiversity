...
# creating a data frame with diferent values for target parameters
df.parameters <- data.frame(mean.beta1 = c(.8, 1.2, 1.8),
                            mean.beta2 = c(.8, 1.2, 1.8),
                            mean.alpha1 = c(.2, .6, .8),
                            mean.alpha2 = c(.2, .6, .8))
...
for (i in 1:length(sim.comm.list)) {
  for (j in 1:length(sim.comm.list[[i]])) {
    ...
    sim.mod[[i]][[j]] <- jagsUI::jags(data = win.data, inits = inits, parameters.to.save = params, 
                                      n.chains = 3,
                                      n.iter = 3000, n.burnin = 100, n.thin = 2, 
                                      model.file = here::here("R", "Bayesian_models", "model_Nmixture_P_RE.txt"))
  }
}
