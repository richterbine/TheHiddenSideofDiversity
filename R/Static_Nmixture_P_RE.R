...
sink(here::here("R", "Bayesian_models", "model_Nmixture_P_RE.txt"))
cat("model
{
  ...
  for(k in 1:nspec){
    ...
    alpha.und[k] ~ dnorm(mu.alpha.can, tau.alpha.can)   # Species-specific random intercept for detectability - understory
